  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-3xl mx-auto">
            {tiers.map((t) => (
              <div key={t.name} className={`p-8 border-2 rounded-2xl ${t.highlighted ? "border-sky-navy bg-sky-navy/[0.02]" : "border-sky-border"}`}>
                ...
                <div className="mt-4">
                  <span className="text-4xl md:text-5xl font-black text-sky-navy">{t.price}</span>
                  <span className="text-sky-muted mr-2">{t.unit}</span>
                </div>
                <ul className="mt-6 space-y-3">
                  {t.features.map((f) => (
                    <li key={f} className="flex items-start gap-2 text-body text-sky-muted">
                      <span className="text-sky-navy mt-0.5">✓</span>{f}
                    </li>
                  ))}
                </ul>
              </div>
      ...
      <ContentSection bg="light">
        ...
        <p className="mt-4 text-body text-sky-muted">שטח התקרה הוא הגורם העיקרי. מעבר לכך: מספר הפינות והחתכים, סוג הגימור (מט, מבריק, סאטן, הדפס), ופתרון התאורה.</p>
        <p className="mt-4 text-body text-sky-muted">תוספות כמו <Link to="/stretch-ceiling-led-strip" className="text-sky-navy hover:underline">פסי LED</Link>, <Link to="/stretch-ceiling-magnetic-track" className="text-sky-navy hover:underline">מסילות מגנטיות</Link> או <Link to="/printed-stretch-ceiling" className="text-sky-navy hover:underline">הדפס מותאם</Link> מתומחרות בנפרד.</p>
        <h2 className="mt-10 text-h2-mobile md:text-h2-desktop">מה כלול במחיר?</h2>
        <p className="mt-4 text-body text-sky-muted">מדידה מקצועית, ייצור בהתאמה אישית, פרופילי אלומיניום, חומרים, התקנה מקצועית, ניקיון באתר, ואחריות. אין תוספות נסתרות.</p>
        <h2 className="mt-10 text-h2-mobile md:text-h2-desktop">מה לא כלול?</h2>
        <p className="mt-4 text-body text-sky-muted">עבודות חשמל (הזזת נקודות חשמל, הוספת שקעים). רכישת גופי תאורה (ספוטים, פסי LED). פירוק תקרה קיימת אם נדרש.</p>
      </ContentSection>
      <ContentSection>
        ...
        <p className="mt-4 text-small text-sky-muted">* מחירים להערכה בלבד. המחיר הסופי נקבע לפי מדידה מדויקת.</p>
      </ContentSection>
      <ContentSection bg="light">
        ...
        <ul className="mt-4 space-y-2 text-body">
          ...
          <li><Link to="/stretch-ceiling-kitchen" className="text-sky-navy hover:underline">תקרה מתוחה למטבח</Link></li>
          <li><Link to="/lighted-stretch-ceiling" className="text-sky-navy hover:underline">תקרה מוארת</Link></li>
          <li><Link to="/stretch-ceiling-vs-drywall" className="text-sky-navy hover:underline">תקרה מתוחה או גבס — השוואת מחירים</Link></li>
        </ul>
      </ContentSection>
      <CTASection title="רוצים הצעת מחיר מדויקת?" text="שלחו לנו את פרטי הפרויקט ונחזור עם הצעה תוך 24 שעות." />
    </>
  );
}
